"""
FinGuardAI - ML Model Training with NSL-KDD Dataset

This script trains the FinGuardAI threat detection model using the NSL-KDD dataset.
It uses the downloaded and processed data to create a Random Forest classifier
that can detect various network threats.
"""
...
from download_nslkdd import prepare_nslkdd_for_training

# Set up logging
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
logger = logging.getLogger('finguardai.ml.training')

def train_threat_detection_model(data_file=None, model_output_path=None):
    """
    Train a threat detection model using NSL-KDD dataset
    
    Args:
        data_file: Path to processed NSL-KDD data (will download if None)
    ...
    # Get dataset (download if needed)
    if data_file and os.path.exists(data_file):
        logger.info(f"Loading existing dataset from {data_file}")
        df = pd.read_csv(data_file)
    else:
        logger.info("Downloading and preparing NSL-KDD dataset...")
        df = prepare_nslkdd_for_training()
        if df is None:
            logger.error("Failed to obtain training data. Exiting.")
            return None, 0
    
    logger.info(f"Dataset loaded with {len(df)} records")
    
    # Feature selection for NSL-KDD data
    features = [
    ...
    # Service is too high cardinality, so we'll use the top 10 most common services
...
def predict_threat(model, features_list, packet_data):
    ...
    protocols = ['protocol_tcp', 'protocol_udp', 'protocol_icmp']
    for protocol in protocols:
        stripped_protocol = protocol.replace('protocol_', '')
        df[protocol] = 1 if packet_data.get('protocol', '') == stripped_protocol else 0
...
if __name__ == "__main__":
    ...
    model, accuracy = train_threat_detection_model()
    ...
    if model is not None:
        ...
        model_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'models')
        features_list = joblib.load(os.path.join(model_dir, 'model_features.joblib'))
        
        # Make a test prediction
        prediction = predict_threat(model, features_list, test_packet)
        logger.info(f"Test prediction on sample packet: {prediction}")
